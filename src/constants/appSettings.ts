...
4. Visual structure preservation`,
	...
	[AppSettingsGroup.FunctionOutput]: {
		manualSaveFileNameFormat: `SpringScreen_{{YYYY-MM-DD_HH-mm-ss}}`,
		autoSaveFileNameFormat: `SpringScreen_{{YYYY-MM-DD_HH-mm-ss}}`,
		fastSaveFileNameFormat: `SpringScreen_{{YYYY-MM-DD_HH-mm-ss}}`,
		focusedWindowFileNameFormat: `${FOCUS_WINDOW_APP_NAME_ENV_VARIABLE}/SpringScreen_{{YYYY-MM-DD_HH-mm-ss}}`,
		fullScreenFileNameFormat: `SpringScreen_{{YYYY-MM-DD_HH-mm-ss}}`,
		videoRecordFileNameFormat: `SpringScreen_Video_{{YYYY-MM-DD_HH-mm-ss}}`,
		uploadToCloudSaveUrlFormat: `SpringScreen_{{YYYY-MM-DD_HH-mm-ss}}`,
	},
	...
	[AppSettingsGroup.SystemCore]: {
		/// 热加载页面数量
		// 默认值为 1：每个热加载页都是一个常驻的 WebView2 渲染进程，
		// 增加会提高内存占用（可在设置中按需调整）
		hotLoadPageCount: 1,
	},
